...
uint8_t RF24::read_register(uint8_t reg, uint8_t* buf, uint8_t len)
{
  ...
  // The status variable should be set to the status byte returned by the command (explained in the datasheet).
	...
	status = SPI.transfer(0b00011111 & reg);
	while(len--){
		*buf++ = SPI.transfer(status);
	}
...
uint8_t RF24::write_register(uint8_t reg, const uint8_t* buf, uint8_t len)
{
  ...
  SPI.beginTransaction(SPISettings(10000000, MSBFIRST, SPI_MODE0));
  digitalWrite(csn_pin, LOW);
  status = SPI.transfer(0b00100000 | (0b00011111 & reg));
...
void RF24::setPALevel(uint8_t level)
{
  ...
  // level can be RF24_PA_MIN, RF24_PA_LOW, RF24_PA_HIGH, or RF24_PA_MAX.
	...
	if (level == 0) {
		write_register(RF_SETUP, setup | 0b00000000);
	} else if (level == 1) {
		write_register(RF_SETUP, setup | 0b00000010);
	} else if (level == 2) {
		write_register(RF_SETUP, setup | 0b00000100);
	} else if (level == 3) {
		write_register(RF_SETUP, setup | 0b00000110);
	}
  // TODO: END HERE
}
...
void RF24::setCRCLength(rf24_crclength_e length)
{
	...
	if (length == RF24_CRC_DISABLED) {
		uint8_t setup = read_register(NRF_CONFIG) & 0b11110111;
		write_register(NRF_CONFIG, setup | 0b00000000);
	} else if(length == RF24_CRC_8) {
		uint8_t setup = read_register(NRF_CONFIG) & 0b11110011;
		write_register(NRF_CONFIG, setup | 0b00001000);
	} else if(length == RF24_CRC_16) {
		uint8_t setup = read_register(NRF_CONFIG) & 0b11110011;
		write_register(NRF_CONFIG, setup | 0b00001100);
	}
    // TODO: END HERE
}
